 ...
 * @method bulkConfiguration(array $defaultConfiguration)
 * @method setCompilationDirectory(string $get_path)
 * @method setFileExtension(string $string)
 * @method setTemplateBaseDirectory(string $string)
 * @method setDefaultFile(string $get_path)
 * @method loadTemplateFile(Closure $param)
 */
class Engine extends ControllerLoader
{
    ...
    public function twigConfig() : Closure
    {
        ...
        return function() use (&$instance)
        {
            ...
            // load compilation cache directory
    ...
    public function defaultConfiguration() : array 
    {
        ...
        // make view manager public
        app()->add('view', $viewManager);

        // make session_token available
        ...
        $getPath = function($target) use (&$base, $controller)
        {
           ...
           // get controller path
           $controllerMainPath = ControllerLoader::getControllerPath($controller);

           // get controller directory
           $controllerDirectory = substr($controllerMainPath, 0, strrpos($controllerMainPath, '/'));

           // can we update path?
           if ($path == constant('CONTROLLER_'.strtoupper($target))) $path = $controllerDirectory . '/' . $path;
